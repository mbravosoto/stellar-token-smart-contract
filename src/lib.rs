...
impl TokenTrait for TokenBDB {
    ...
    // Fuction that gives value to a token
    // Just the admin can create new tokens: Trust and security of the ecosystem
    // Each operation must be secure, auditable and verificable
    ...
    // Most frequent operation in a token
    // Must be extremely efficient, secure and clear
    // Each transfer modifies two balances atomically: substracts from source and adds to destination
    // Guarantee funds are transfered correctly in the Blockchain
    fn transfer(
        env: Env, 
        from: Address, 
        to: Address, 
    ...
    fn transfer_from(
        ...
        env.events().publish(
            (symbol_short!("trnsfrfrm"), spender, from.clone(), to.clone()), // Allows max 9 characters: trnsfrfrm
